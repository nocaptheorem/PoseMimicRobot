...
namespace NoCAPTheorem.Virtual
{
  ...
  public partial class ActiveRagdoll : Skeleton3D
  {
    ...
    [Export] public float PoseBlendSpeed = 4.0f; // Controls how fast stiffness adapts
    #endregion

    // --- INTERNAL STATE ---
    private float _airborneTimer = 0.0f;
    private float _currentStiffnessMult = 1.0f; // Tracks the smoothed multiplier
...
private void ToggleShadowVisibility()
    ...
    public override void _PhysicsProcess(double delta)
    {
      ...
      // --- DYNAMIC STIFFNESS BLENDING ---
      float targetStiffness = 0.4f; // Default standing
      if (isFallen) targetStiffness = 0.2f;
      else if (isAirborne) targetStiffness = 0.05f;

      // Exponential decay smoothing (frame-rate independent via dt)
      _currentStiffnessMult = Mathf.Lerp(_currentStiffnessMult, targetStiffness, PoseBlendSpeed * dt);
      ...
      if (isFallen)
      {
        ...
        // Use the smoothed multiplier here
        MatchShadowPose(dt, false, _currentStiffnessMult, new HashSet<int>());
        VirtualModelControl(dt, isDebugFrame, true);
      }
      else if (isAirborne)
      {
        // Use the smoothed multiplier here
        MatchShadowPose(dt, false, _currentStiffnessMult, stanceBoneIds);
        CompensateForGravity(dt);
      }
      else
      {
        StiffenSpineAtHipsAndChest(dt);
        CompensateForGravity(dt);

        // Use the smoothed multiplier here
        MatchShadowPose(dt, isDebugFrame, _currentStiffnessMult, stanceBoneIds);
        StabilizeCore(dt);
        VirtualModelControl(dt, isDebugFrame, false);
        AnkleControl(dt);
      }
    ...
    private void StiffenSpineAtHipsAndChest(float dt)
    {
      if (_chest == null || _hips == null) return;
      float extensionStrength = 9000.0f;
      Vector3 force = Vector3.Up * extensionStrength;
    ...
    private void MatchShadowPose(float dt, bool logDebug, float strengthMult, HashSet<int> activeStanceBones)
    {
      foreach (var m in _muscles)
      {
        if (m.Bone == _hips && !_wasFallen) continue;

        float currentStrengthMult = strengthMult;
        if (_grabbedLimbBoneIds.Contains(m.BoneId))
        {
          currentStrengthMult = 0.01f;
        }
        ...
        if (activeStanceBones.Contains(m.BoneId))
        {
          ...
          if (m.IsLeg && kneeExtension > 0.9f) {
            stiffness *= 0.3f;
            damping *= 0.3f; // Free the joint to move
          }
          else {
            stiffness *= 0.05f;
            damping *= 0.1f; // Free the joint to move
          }
        }

        if (m.IsSpine) { stiffness *= 10.0f; damping *= 2.0f; }
        else if (m.IsHead) stiffness *= 2.0f;
        else if (m.IsArm) {
          if (RelaxArms) {
            stiffness = 0.0f;
            damping = MuscleDamping * 0.6f; // High friction so the arm acts heavy and absorbs momentum
          } else {
            stiffness *= 0.8f;
            damping *= 1.0f;
          }
    ...
    private void StabilizeCore(float dt)
    {
      if (_hips == null || _hipMuscle == null) return;

      Transform3D hipVisualWorld = this.GlobalTransform * this.GetBoneGlobalPose(_hipMuscle.BoneId);
      Vector3 hipForward = -hipVisualWorld.Basis.Z;
      Vector3 currentUp = hipVisualWorld.Basis.Y;
      ...
      foreach(var leg in _legs) {
        if (leg.GroundSensor.IsColliding()) {
          avgFootPos += leg.Foot.GlobalPosition;
          footCount++;
        }
      }
      ...
      if (footCount > 0) {
        avgFootPos /= footCount;
        Vector3 balanceError = com - avgFootPos;
      ...
      // 1. Upright Stabilization (Pitch & Roll)
      ...
      // 2. Heading Stabilization (Yaw)
      if (AnimationShadow != null) {
        Transform3D shadowHipWorld = AnimationShadow.GlobalTransform * AnimationShadow.GetBoneGlobalPose(_hipMuscle.BoneId);
        Vector3 targetForward = -shadowHipWorld.Basis.Z;

        // Project forwards onto the XZ plane to isolate the yaw rotation
        Vector3 currentHeading = new Vector3(hipForward.X, 0, hipForward.Z).Normalized();
        Vector3 targetHeading = new Vector3(targetForward.X, 0, targetForward.Z).Normalized();

        Vector3 yawAxis = currentHeading.Cross(targetHeading).Normalized();
        float yawAngle = Mathf.Acos(Mathf.Clamp(currentHeading.Dot(targetHeading), -1f, 1f));

        if (yawAngle > 0.001f) {
          totalTorque += yawAxis * (yawAngle * HipGyroStiffness);
        }
      }

      // Apply combined proportional torque minus the angular velocity damping
      if (totalTorque.LengthSquared() > 0.0001f) {
        Vector3 finalTorque = totalTorque - (_hips.AngularVelocity * HipGyroDamping);
        ApplyTorque(_hips, finalTorque, dt);
      }
    }

private void VirtualModelControl(float dt, bool logDebug, bool isFallen)
    {
      if (_hips == null) return;
      Vector3 centerOfMass = GetTrueCenterOfMass();
      Vector3 hipVel = _hips.LinearVelocity;

      float currentRelativeHeight = TargetHeight;
      ...
      {
        hasGroundBelow = true;
        currentRelativeHeight = _hips.GlobalPosition.Y - result["position"].AsVector3().Y;
      }
      ...
      if (isFallen) {
        // Only apply the "get up" recovery force if there is actually a floor to push off of!
        if (hasGroundBelow) {
            ApplyForce(_hips, Vector3.Up * verticalForceMag, dt);
        }
        return;
      }

      // Filter legs by hysteresis threshold rather than a binary collision check
      _groundedLegsCache.Clear();
      foreach (var leg in _legs) {
        if (leg.GroundedConfidence > 0.01f) {
          _groundedLegsCache.Add(leg);
        }
      }
      if (_groundedLegsCache.Count == 0) return;

      // 1. Calculate a CONFIDENCE-WEIGHTED average foot position (Center of Pressure)
      Vector3 avgFootPos = Vector3.Zero;
      float totalConfidence = 0f;

      foreach(var leg in _groundedLegsCache) {
        int footBoneId = leg.ChainBoneIds[2];
        Transform3D footVisualWorld = this.GlobalTransform * this.GetBoneGlobalPose(footBoneId);

        Vector3 footForward = -footVisualWorld.Basis.Z;
        footForward.Y = 0;
        footForward = footForward.Normalized();

        // Multiply the foot's contribution by its confidence
        avgFootPos += (leg.Foot.GlobalPosition + (footForward * CenterOfPressureOffset)) * leg.GroundedConfidence;
        totalConfidence += leg.GroundedConfidence;
      }
      avgFootPos /= totalConfidence;

      Vector3 horizontalError = avgFootPos - centerOfMass;
      horizontalError.Y = 0;
      ...
      if (desiredBodyForce.Length() > MaxForce) desiredBodyForce = desiredBodyForce.Normalized() * MaxForce;
      Vector3 footReactionForce = -desiredBodyForce;

      var legWeights = new Dictionary<LimbChain, float>();

      // 2. FIRST: Calculate the raw geometric weights based on the Center of Mass projection
      if (_groundedLegsCache.Count == 2)
      {
        Vector3 L = _groundedLegsCache[0].Foot.GlobalPosition;
        Vector3 R = _groundedLegsCache[1].Foot.GlobalPosition;
        Vector3 line = R - L;
        float lenSq = line.LengthSquared();
        if (lenSq > 0.001f)
        {
          float t = (centerOfMass - L).Dot(line) / lenSq;
          t = Mathf.Clamp(t, 0f, 1f);
          legWeights[_groundedLegsCache[0]] = 1.0f - t;
          legWeights[_groundedLegsCache[1]] = t;
        }
        else { legWeights[_groundedLegsCache[0]] = 0.5f; legWeights[_groundedLegsCache[1]] = 0.5f; }
      }
      else
      {
        float totalWeight = 0f;
        Vector3 com2D = new Vector3(centerOfMass.X, 0, centerOfMass.Z);
        foreach (var leg in _groundedLegsCache) {
          Vector3 fPos = leg.Foot.GlobalPosition;
          float dist = new Vector3(fPos.X, 0, fPos.Z).DistanceTo(com2D);
          float w = 1.0f / (dist + 0.1f);
          legWeights[leg] = w;
          totalWeight += w;
        }
        foreach (var leg in _groundedLegsCache) legWeights[leg] /= totalWeight;
      }

      // 3. SECOND: Blend the calculated geometric weights with our physical confidence
      float finalWeightSum = 0f;
      foreach (var leg in _groundedLegsCache)
      {
        legWeights[leg] *= leg.GroundedConfidence;
        finalWeightSum += legWeights[leg];
      }

      // 4. THIRD: Normalize so they sum back to 1.0
      foreach (var leg in _groundedLegsCache)
      {
        legWeights[leg] = finalWeightSum > 0.001f ? (legWeights[leg] / finalWeightSum) : (1.0f / _groundedLegsCache.Count);
      }

      // 5. FOURTH: Apply the torques
      foreach (var leg in _groundedLegsCache) {
        float share = legWeights.ContainsKey(leg) ? legWeights[leg] : (1.0f / _groundedLegsCache.Count);
        Vector3 legForce = footReactionForce * share;

        // Get true forward vector from visual foot bone
        int footBoneId = leg.ChainBoneIds[2];
        Transform3D footVisualWorld = this.GlobalTransform * this.GetBoneGlobalPose(footBoneId);
        ...
        Vector3 hipTorque = r_Hip.Cross(legForce);
        Vector3 kneeTorque = r_Knee.Cross(legForce);
        Vector3 ankleTorque = r_Ankle.Cross(legForce);

        if (hipTorque.Length() > MaxMuscleTorque) hipTorque = hipTorque.Normalized() * MaxMuscleTorque;
        if (kneeTorque.Length() > MaxMuscleTorque) kneeTorque = kneeTorque.Normalized() * MaxMuscleTorque;
        if (ankleTorque.Length() > MaxMuscleTorque) ankleTorque = ankleTorque.Normalized() * MaxMuscleTorque;
    ...
    private void SwingHipRecovery(float dt)
    {
    ...
    private void AnkleControl(float dt)
    {
      foreach (var leg in _legs) {
        Vector3 groundNormal = Vector3.Up;
        if (leg.GroundSensor.IsColliding()) groundNormal = leg.GroundSensor.GetCollisionNormal(0);

        // Get true up vector from visual foot bone
        int footBoneId = leg.ChainBoneIds[2];
        Transform3D footVisualWorld = this.GlobalTransform * this.GetBoneGlobalPose(footBoneId);
        Vector3 footUp = footVisualWorld.Basis.Y;

        Vector3 rotAxis = footUp.Cross(groundNormal).Normalized();
